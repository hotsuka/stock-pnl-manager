...
#### IRR（内部収益率）計算
- **保有銘柄のIRR**: 投資パフォーマンスをIRRで評価
- **売却済銘柄のIRR**: 確定損益のIRRを自動計算
- **XIRR方式**: 不定期のキャッシュフローに対応した精密計算

#### 取引管理の強化
- **手動入力**: 個別取引を直接入力可能
- **ソート機能**: 各種テーブルで列ソート対応
- **Stickyヘッダー**: スクロール時もヘッダーを固定表示
...
CSVファイルを使わずに、個別に取引を登録することもできます：

1. **アップロードページにアクセス**
   - `http://localhost:8000/upload` にアクセス
   - 「個別入力」タブを選択

2. **取引情報を入力**
   - 取引種別（買付/売却）を選択
   - 取引日を入力
   - ティッカーシンボルを入力（例: AAPL, 9984.T）
   - 銘柄名（任意）
   - 数量、単価、受渡金額を入力
   - 通貨を選択（自動推定も可）
...
| IRR | 内部収益率（年率換算のパフォーマンス指標） |
| 前日比 | 前営業日からの変動率 |
| 通貨 | 取引通貨（JPY, USD, KRW） |

#### テーブルのソート機能

保有銘柄一覧のテーブルでは、列ヘッダーをクリックすることでソートできます：
- ティッカー、銘柄名でアルファベット順/逆順
- 取得コスト、評価額、損益、IRRなどで数値順/逆順
- Stickyヘッダーにより、スクロールしてもヘッダーは常に表示
...
年度別の配当金額をクリックすると、その銘柄・年度の配当履歴詳細がモーダルで表示されます：
- 権利落ち日
- 1株あたり配当額
- 保有株数
- 受取配当金額
...
| 確定損益 | 売却代金 - 総取得コスト |
| 損益率 | 確定損益 / 総取得コスト × 100 |
| IRR | 内部収益率（取得から売却までの年率パフォーマンス） |
| 通貨 | 株式の通貨 |

#### IRR（内部収益率）について

IRRは、投資の年率換算パフォーマンスを示す指標です：
- 保有期間に関係なく、年率換算で比較可能
- プラスは利益、マイナスは損失
- 配当を含めた総合的なリターンを評価

**計算方法**:
XIRR（拡張内部収益率）方式を使用し、不定期なキャッシュフロー（購入、売却、配当）に対応しています。
