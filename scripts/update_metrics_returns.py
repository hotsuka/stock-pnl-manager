...
def update_returns():
    ...
    with app.app_context():
        ...
        print(
            f"銘柄: {', '.join(ticker_symbols[:10])}{'...' if len(ticker_symbols) > 10 else ''}"
        )
        ...
        if results["success"] > 0:
            ...
            for m in sample_metrics:
                print(f"\n{m.ticker_symbol}:")
                print(
                    f"  YTDリターン: {float(m.ytd_return) if m.ytd_return else 'データなし'}"
                )
                print(
                    f"  1年リターン: {float(m.one_year_return) if m.one_year_return else 'データなし'}"
                )
                if m.ytd_return:
